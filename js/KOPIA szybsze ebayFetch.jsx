            ...
            .then(data => {
                ...
                // if (this.state.queryParams.pageNumber<totalPages) {
                    this.setState({
                        ...
                        new Array(this.state.queryParams.totalPages-1).fill(undefined).forEach((elem, index) => {
                            this.fetchNextPage(index+2);
                        });
                    });
                // } else {
                //     this.setState({
    ...
    fetchNextPage = (pageNumber) => {
        fetch(prepareUrl({
            ...
            .then(data => {
                const items = data.searchResult[0].item;
                const totalPages = Number(data.paginationOutput[0].totalPages[0]);
                if (pageNumber !== this.state.queryParams.totalPages) {
                    this.setState({
                        itemList: [...this.state.itemList, ...items],
                    });
                } else {
    ...
    createArrayWithPrices = () => {
        ...
        return (
            this.state.itemList.map(item => {
                console.log(item);
                return Number(item.shippingInfo[0].shippingServiceCost[0].__value__) + Number(item.sellingStatus[0].currentPrice[0].__value__)
            })
        );
    }
        ...
        return (
                ...
                {loading  && 
                    <div>
                        ...
                        <p className="loadingParagraph">{((this.state.itemList.length)/this.state.queryParams.totalPages).toFixed(2)} %</p>
                    </div>}
                {(!loading && this.state.itemList.length!==0) &&
                    <div>
                        <AllStats 
